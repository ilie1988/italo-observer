...
@app.template_filter('italo')
def format_italo(atomic, tag=True, fixed=False, decimals=9, zero=None):
    """Formats an atomic current value as a human currency value.
    tag - if False then don't append " ITALO"
...
def get_sns_future(lmq, italod):
    return FutureJSON(lmq, italod, 'rpc.get_service_nodes', 5,
            args={
                'all': False,
...
def get_quorums_future(lmq, italod, height):
    return FutureJSON(lmq, italod, 'rpc.get_quorum_state', 30,
...
def get_mempool_future(lmq, italod):
    return FutureJSON(lmq, italod, 'rpc.get_transaction_pool', 5, args={"tx_extra":True})
...
def main(refresh=None, page=0, per_page=None, first=None, last=None):
    lmq, italod = lmq_connection()
    inforeq = FutureJSON(lmq, italod, 'rpc.get_info', 1)
    stake = FutureJSON(lmq, italod, 'rpc.get_staking_requirement', 10)
    base_fee = FutureJSON(lmq, italod, 'rpc.get_fee_estimate', 10)
    hfinfo = FutureJSON(lmq, italod, 'rpc.hard_fork_info', 10)
    mempool = get_mempool_future(lmq, italod)
    sns = get_sns_future(lmq, italod)
    checkpoints = FutureJSON(lmq, italod, 'rpc.get_checkpoints', args={"count": 3})

    # This call is slow the first time it gets called in italod but will be fast after that, so call
    ...
    coinbase = FutureJSON(lmq, italod, 'admin.get_coinbase_tx_sum', 10, timeout=1, fail_okay=True,
    ...
    blocks = FutureJSON(lmq, italod, 'rpc.get_block_headers_range', cache_key='main', args={
    ...
    if blocks and 'txs' not in blocks[0]:
        ...
        for b in blocks:
            ...
            if 'tx_hashes' in b:
                txids += b['tx_hashes']
        txs = parse_txs(tx_req(lmq, italod, txids, cache_key='mempool').get())
        i = 0
        for tx in txs:
...
def mempool():
    lmq, italod = lmq_connection()
    info = FutureJSON(lmq, italod, 'rpc.get_info', 1)
    mempool = get_mempool_future(lmq, italod)
...
def sns():
    lmq, italod = lmq_connection()
    info = FutureJSON(lmq, italod, 'rpc.get_info', 1)
    awaiting, active, inactive = get_sns(get_sns_future(lmq, italod), info)
...
def tx_req(lmq, italod, txids, cache_key='single', **kwargs):
    return FutureJSON(lmq, italod, 'rpc.get_transactions', cache_seconds=10, cache_key=cache_key,
            args={
                "txs_hashes": txids,
                "decode_as_json": True,
                "tx_extra": True,
...
def sn_req(lmq, italod, pubkey, **kwargs):
    return FutureJSON(lmq, italod, 'rpc.get_service_nodes', 5, cache_key='single',
            args={"service_node_pubkeys": [pubkey]}, **kwargs
        )


def block_header_req(lmq, italod, hash_or_height, **kwargs):
    if isinstance(hash_or_height, int) or (len(hash_or_height) <= 10 and hash_or_height.isdigit()):
        return FutureJSON(lmq, italod, 'rpc.get_block_header_by_height', cache_key='single',
                args={ "height": int(hash_or_height) }, **kwargs)
    else:
        return FutureJSON(lmq, italod, 'rpc.get_block_header_by_hash', cache_key='single',
                args={ 'hash': hash_or_height }, **kwargs)


def block_with_txs_req(lmq, italod, hash_or_height, **kwargs):
    ...
    return FutureJSON(lmq, italod, 'rpc.get_block', cache_key='single', args=args, **kwargs)
...
def show_sn(pubkey):
    ...
    info = FutureJSON(lmq, italod, 'rpc.get_info', 1)
    hfinfo = FutureJSON(lmq, italod, 'rpc.hard_fork_info', 10)
    sn = sn_req(lmq, italod, pubkey).get()
...
def parse_txs(txs_rpc):
    ...
    for tx in txs_rpc['txs']:
        if 'info' not in tx:
            # We have serialized JSON data inside a field in the JSON, because of italod's
...
def get_block_txs_future(lmq, italod, block):
    hashes = []
    ...
    return tx_req(lmq, italod, hashes, cache_key='block')
...
def show_block(height=None, hash=None, more_details=False):
    ...
    block = None if val is None else block_with_txs_req(lmq, italod, val).get()
    if block is None:
        return flask.render_template("not_found.html",
                info=info.get(),
                hfinfo=hfinfo.get(),
    ...
    txs = get_block_txs_future(lmq, italod, block)

    if info.get()['height'] > 1 + block_height:
        next_block = block_header_req(lmq, italod, '{}'.format(block_height + 1))
...
def show_block_latest():
    lmq, italod = lmq_connection()
    height = FutureJSON(lmq, italod, 'rpc.get_info', 1).get()['height'] - 1
...
def show_tx(txid, more_details=False):
    lmq, italod = lmq_connection()
    info = FutureJSON(lmq, italod, 'rpc.get_info', 1)
    txs = tx_req(lmq, italod, [txid]).get()
    ...
    if tx['info']['version'] >= 4 and 'sn_state_change' in tx['extra']:
        testing_quorum = FutureJSON(lmq, italod, 'rpc.get_quorum_state', 60, cache_key='tx_state_change',
    ...
    if 'vin' in tx['info']:
        if len(tx['info']['vin']) == 1 and 'gen' in tx['info']['vin'][0]:
            ...
            outputs = FutureJSON(lmq, italod, 'rpc.get_outs', args={
                'get_txid': True,
                'outputs': outs_req,
                }).get()
            if outputs and 'outs' in outputs and len(outputs['outs']) == len(outs_req):
                ...
                block_info_req = FutureJSON(lmq, italod, 'rpc.get_block_header_by_height', args={
...
def show_quorums():
    lmq, italod = lmq_connection()
    info = FutureJSON(lmq, italod, 'rpc.get_info', 1)
    quos = get_quorums_future(lmq, italod, info.get()['height'])
...
def search():
    ...
    snreq = sn_req(lmq, italod, val)
    blreq = block_header_req(lmq, italod, val, fail_okay=True)
    txreq = tx_req(lmq, italod, [val])
...
def api_block(blkid=None, height=None):
    lmq, italod = lmq_connection()
    block = block_with_txs_req(lmq, italod, blkid if blkid is not None else height).get()
    txs = get_block_txs_future(lmq, italod, block)
